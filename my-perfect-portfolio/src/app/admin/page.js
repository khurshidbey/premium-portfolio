  ...
  const handleProjectSubmit = async (e) => {
    ...
    try {
      ...
      await databases.createDocument(
        ...
        process.env.NEXT_PUBLIC_APPWRITE_PROJECTS_COLLECTION_ID,
        ID.unique(),
        { title, description, github_link: link, image_url: imageUrl, pdf_url: pdfUrl, gallery_urls: galleryUrlsArray.length > 0 ? JSON.stringify(galleryUrlsArray) : null, category }
      );
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6 border-b border-white/10 pb-6">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
            <button onClick={() => setActiveTab("projects")} className={`px-6 py-2 rounded-lg font-semibold flex items-center gap-2 transition-all ${activeTab === "projects" ? 'bg-blue-500/20 text-blue-400' : 'text-gray-400 hover:text-white'}`}><FolderKanban size={18}/> Loyihalar</button>
            <button onClick={() => setActiveTab("prompts")} className={`px-6 py-2 rounded-lg font-semibold flex items-center gap-2 transition-all ${activeTab === "prompts" ? 'bg-purple-500/20 text-purple-400' : 'text-gray-400 hover:text-white'}`}><Sparkles size={18}/> AI Prompts</button>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
            ...
            {activeTab === "projects" ? (
              <>
                ...
                <form onSubmit={handleProjectSubmit} className="flex flex-col gap-4">
                  ...
                  <div className="border-2 border-dashed border-white/20 p-4 rounded-xl text-center bg-black/30"><p className="text-xs text-gray-400 mb-2">Asosiy Rasm</p><input type="file" accept="image/*" onChange={(e) => setImage(e.target.files[0])} className="text-xs text-gray-500 cursor-pointer" /></div>
                  <div className="border border-white/10 p-3 rounded-xl bg-black/30"><p className="text-xs text-gray-400 mb-2">Galereya</p><input type="file" multiple accept="image/*" onChange={(e) => setGallery(Array.from(e.target.files))} className="text-xs text-gray-500 cursor-pointer" /></div>
                  <div className="border border-white/10 p-3 rounded-xl bg-black/30"><p className="text-xs text-gray-400 mb-2">PDF</p><input type="file" accept=".pdf" onChange={(e) => setPdfFile(e.target.files[0])} className="text-xs text-gray-500 cursor-pointer" /></div>
                  <button type="submit" disabled={loading} className={`mt-2 py-4 rounded-xl font-bold text-white ${loading ? 'bg-gray-600' : 'bg-gradient-to-r from-blue-600 to-blue-400'}`}>{loading ? "Kuting..." : "Saqlash"}</button>
                </form>
              </>
            ) : (
              <>
                <h2 className="text-xl font-semibold mb-6 text-purple-400 flex items-center gap-2"><Sparkles/> Yangi Prompt</h2>
                <form onSubmit={handlePromptSubmit} className="flex flex-col gap-4">
                  ...
                  <textarea placeholder="Prompt matni..." required rows="6" value={promptText} onChange={(e) => setPromptText(e.target.value)} className="w-full bg-black/50 border border-white/10 p-4 rounded-xl outline-none focus:border-purple-500 resize-none font-mono text-sm text-gray-300"></textarea>
                  <div className="border-2 border-dashed border-white/20 p-5 rounded-xl text-center bg-black/30"><p className="text-xs text-gray-400 mb-2">Rasm yuklang</p><input type="file" accept="image/*" onChange={(e) => setPromptImage(e.target.files[0])} className="text-xs text-gray-500 cursor-pointer" /></div>
                  <button type="submit" disabled={loading} className={`mt-2 py-4 rounded-xl font-bold text-white ${loading ? 'bg-gray-600' : 'bg-gradient-to-r from-purple-600 to-pink-500'}`}>{loading ? "Kuting..." : "Saqlash"}</button>
                </form>
              </>
            )}
          </motion.div>
          ...
          <motion.div key={activeTab + "-list"} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-md flex flex-col">
            <h2 className="text-xl font-semibold mb-6 flex justify-between items-center">
              Ro'yxat <span className="py-1 px-3 rounded-full text-sm bg-white/10">{activeTab === "projects" ? projects.length : promptsList.length}</span>
            </h2>
            <div className="flex flex-col gap-4 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
              {activeTab === "projects" ? (
                projects.length === 0 ? <p className="text-gray-500 text-center py-10">Yo'q</p> :
                projects.map(item => (
                  <div key={item.$id} className="flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-4 overflow-hidden">
                      ...
                      <div><h3 className="font-bold text-white truncate max-w-[150px]">{item.title}</h3><span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">{item.category}</span></div>
                    </div>
                    <button onClick={() => handleDeleteProject(item.$id)} className="p-3 bg-red-500/10 text-red-400 rounded-xl"><Trash2 size={16} /></button>
                  </div>
                ))
              ) : (
                promptsList.length === 0 ? <p className="text-gray-500 text-center py-10">Yo'q</p> :
                promptsList.map(item => (
                  <div key={item.$id} className="flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-4 overflow-hidden">
                      <div className="w-14 h-14 rounded-xl overflow-hidden bg-white/5 flex-shrink-0"><img src={item.image_url} className="w-full h-full object-cover" /></div>
                      <div><h3 className="font-bold text-white truncate max-w-[150px]">{item.title}</h3></div>
                    </div>
                    <button onClick={() => handleDeletePrompt(item.$id)} className="p-3 bg-red-500/10 text-red-400 rounded-xl"><Trash2 size={16} /></button>
                  </div>
                ))
              )}
            </div>
